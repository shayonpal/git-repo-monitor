...
# Function to check if a repo needs pulling with timeout
repo_needs_pull() {
  local repo="$1"
  ...
  # Fetch updates from remote with a timeout
  echo "DEBUG: Fetching updates for $repo" >&2
  timeout 5s git fetch --quiet 2>/dev/null || { echo "DEBUG: Failed to fetch updates for $repo" >&2; return 1; }
  ...
  echo "$status" | grep -E 'Your branch is behind|have diverged' >&2 && { echo "DEBUG: $repo needs pull" >&2; return 0; } || { echo "DEBUG: $repo is up to date" >&2; return 1; }
}
...
main() {
  ...
  local needs_pull=()
  local count=0
  local max_repos=10  # Limit how many repos to check to avoid timeouts
...
# Run the script
main 2>/tmp/git_monitor_debug2.log